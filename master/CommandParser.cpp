...
// Hilfsfunktion für Case-insensitive String-Vergleich
...
void parseCommand(const char *input)
{
  ...
  if (strcasecompare(command, "UPDATE") == 0)
  {
    handleUpdateCommand(values, valueCount);
  }
  else if (strcasecompare(command, "SCAN") == 0)
  {
    handleScanCommand();
  }
  else if (strcasecompare(command, "STOP") == 0)
  {
    handleStopCommand(values, valueCount);
  }
  else if (strcasecompare(command, "LED") == 0)
  {
    handleLEDCommand(values, valueCount);
  }
  else
...
>>>>>>> 541b617 (Verbessere die Fallunempfindlichkeit der Befehlsverarbeitung und optimiere die Hauptschleife zur Handhabung von seriellen Eingaben.):Master/CommandParser.cpp
